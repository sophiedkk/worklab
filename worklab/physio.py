...
def aerobic_threshold(data_spiro, power, start_spiro, muser):
    ...
    ax[0, 0].scatter(data_spiro["VO2"].rolling(window=15).mean(), data_spiro["HR"].rolling(window=15).mean(),
                     color='red', s=1)
    ax[0, 0].set_ylabel("HR [b/min]", {'color': 'red'})
    ax[0, 0].set_ylim(0, 220)
    ax[0, 0].set_xlabel("VO2 [l/min]")
    ax1 = ax[0, 0].twinx()
    ax1.plot(data_spiro["VO2"].rolling(window=15).mean(), data_spiro["VCO2"].rolling(window=15).mean(),
             color='darkblue')
    ax1.set_ylabel("VCO2 [l/min]", {'color': 'darkblue'})
    ...
    vt1 = pd.DataFrame([{'VT1-time': time - int(start_spiro), 'VT1-power': power.iloc[time].values[0],
                         ...
                         'VT1-VO2/kg': ((data_spiro['VO2'].rolling(window=30).mean()[data_spiro['time'] == time].values[
                             0]) * 1000) / muser}])
...
def anaerobic_threshold(data_spiro, power, start_spiro, muser):
    ...
    vt2 = pd.DataFrame([{'VT2-time': time - int(start_spiro), 'VT2-power': power.iloc[time].values[0],
                         ...
                         'VT2-VO2/kg': ((data_spiro['VO2'].rolling(window=30).mean()[data_spiro['time'] == time].values[
                             0]) * 1000) / muser}])
